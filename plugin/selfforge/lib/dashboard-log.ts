...
/** Business data types shown in the dashboard (memory/skill/rule/goal/…), excluding tool & dashboard chrome. */
const BUSINESS_TYPES = new Set([
  "memory", "skill", "rule", "goal", "checkpoint", "evolution", "repair",
  "pattern", "workspace", "transfer", "team", "daily", "session",
])

/** Get call stats: counters, recent, per-day breakdown. Only business data types. */
export function getCallStats(): {
  ...
  try {
    ...
    const biz = all.filter((r) => BUSINESS_TYPES.has(r.type))
    recent = biz.slice(-50).reverse()
    ...
    for (const r of biz) rebuilt[r.type] = (rebuilt[r.type] || 0) + 1
    return { counters: rebuilt, recent, byDay, totalCalls: all.length }
  } catch {}
  // Filter out non-business keys even in the in-memory fallback.
  const filteredCounters: Record<string, number> = {}
  for (const [k, v] of Object.entries(counters)) if (BUSINESS_TYPES.has(k)) filteredCounters[k] = v
  return {
    counters: filteredCounters,
    recent: recent.filter((r) => BUSINESS_TYPES.has(r.type)),
    byDay: {},
